...
class PairingDialog(NavWidget):
  ...
  def __init__(self):
    ...
    self._pair_label = UnifiedLabel("pair with stable.konik.ai" if self._params.get_int("EnableConnect") == 3 else "pair with comma connect",
                                    font_size=48, font_weight=FontWeight.BOLD, line_height=0.8)
  ...
  def _get_pairing_url(self) -> str:
    try:
      ...
      token = ""
    host = "https://stable.konik.ai" if self._params.get_int("EnableConnect") == 3 else "https://connect.comma.ai"
  ...
  def _update_state(self):
    super()._update_state()
    if ui_state.prime_state.is_paired() and self._params.get_int("EnableConnect") != 3 and not self.is_dismissing:
      self.dismiss()
